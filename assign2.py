...
# The remove_unreachable_state() function calls the traversing() function and then uses the updated list of reachable states 
...
# The minimizing DFA function below creates the equivalance classes and returns them as output
...
# The equivalance classes returned from minimizing dfa function is used in creat_dfa_table() function
# to create new dfa table and give tuple values.
...
choice = input('Do you want to put data manually or the data should be taken from dfa_data.py file?(Yes to put manually No to read data from file).  ')
if choice.lower() == 'yes':
    DFA_table, start_state, set_of_states_Q, set_of_final_states_F = input_data()
else:
    import dfa_data
    DFA_table = dfa_data.DFA_table
    start_state = dfa_data.start_state
    set_of_states_Q = dfa_data.set_of_states_Q
    set_of_final_states_F = dfa_data.set_of_final_states_F
